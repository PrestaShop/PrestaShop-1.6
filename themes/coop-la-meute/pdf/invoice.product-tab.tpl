...
<table class="product" width="100%" cellpadding="4" cellspacing="0">
	...
	<thead>
	<tr>
		{* Nicolas MAURENT - 08.04.17 - Replacing reference by stock and ID *}
		<th class="product header small" width="{$layout.stock.width}%">{l s='Stock' pdf='true'}</th>
		<th class="product header small" width="{$layout.id.width}%">{l s='ID' pdf='true'}</th>
		<!--th class="product header small" width="{$layout.reference.width}%">{l s='Reference' pdf='true'}</th-->
		{* Nicolas MAURENT - 08.04.17 - End *}
	...
	{foreach $order_details as $order_detail}
		...
		<tr class="product {$bgcolor_class}">
			...
			<td class="product left">
				{if substr($order_detail.product_reference,-1)=="S"}<big>&#x2610;</big><small> stock</small>{/if}
			</td>
