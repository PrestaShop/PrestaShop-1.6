...
<table width="100%" id="body" border="0" cellpadding="0" cellspacing="0" style="margin:0;">
	...
	<tr>
		{* Nicolas MAURENT - 04.02.17 - Changing size from 7 to 6 *}
	...
	{* Nicolas MAURENT - 04.02.17 - Adding bloc for messages *}
	<tr>
		<td colspan="12" height="10">&nbsp;</td>
	</tr>
	...
	<tr>
		<td colspan="12">
			...
			<table class="message" width="100%" cellpadding="4">
				<thead>
					<tr>
						<th class="message header small" colspan="3">Messages ({$messages|@count}):</th>
						<th class="message header small" colspan="9">&nbsp;</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr class="{if $smarty.foreach.messageList.first}first_item{elseif $smarty.foreach.messageList.last}last_item{/if} {if $smarty.foreach.messageList.index % 2}alternate_item{else}item{/if}">
						<td class="{if $message@iteration is even by 1}grey{else}white{$name}{/if}" colspan="3">
							<strong class="dark">
						...
						<td class="{if $message@iteration is even by 1}grey{else}white{$name}{/if}" colspan="9">{$message.message|escape:'html':'UTF-8'|nl2br}</td>
					</tr>
					{/foreach}
				</tbody>
			</table>
